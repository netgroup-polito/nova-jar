...
class DockerGenericVIFDriver(object):
    ...
    def attach(self, instance, vif, container_id):
        vif_type = vif['type']
        if container_id not in self.container_vifs_num:
	    new_name = 'eth0'
	    self.container_vifs_num[container_id]=1
	else:
	    new_name = 'eth'+str(self.container_vifs_num[container_id])
	    self.container_vifs_num[container_id]+=1

	LOG.audit("Setting interface: " + new_name)
        ...
        try:
            ...
            utils.execute('ip', 'netns', 'exec', container_id, 'ip', 'link', 
            			  'set', 'dev', if_remote_name, 'name', new_name, 
            		       run_as_root=True)
            if_remote_name = new_name
            utils.execute('ip', 'netns', 'exec', container_id, 'ip', 'link',
    ...
    def end_of_attach(self, container_id):
		del self.container_vifs_num[container_id]
