...
docker_opts = [
    ...
    cfg.StrOpt('snapshots_directory',
               ...
               help='Location where docker driver will temporarily store '
                    'snapshots.'),
    cfg.StrOpt('jar_images_directory',
               default='$instances_path/jarimg',
               help='Location where docker driver will mount volume inside container.'),
    cfg.StrOpt('docker_image_name',
               default='sebymiano92/javaubuntu',
               help='Name of the container where the jar will be inserted.'),
    cfg.StrOpt('docker_jar_path',
               default='/bin'
               help='Location inside the Docker container where put the downloaded jar.')
]
...
class DockerDriver(driver.ComputeDriver):
	...
	self.vif_driver.end_of_attach(container_id)
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        image = self.docker.inspect_image()
        if not image:
            raise exception.InstanceDeployFailure(_('Cannot found container'), instance_id=instance['name'])

        container_id = self._create_container(instance, self._encode_utf8(CONF.docker.docker_image_name), args)
        if not container_id:
            raise exception.InstanceDeployFailure(_('Cannot create container'), instance_id=instance['name'])

	self._start_container(container_id, instance, network_info)

	full_name = image_name + ".jar"
	
	if(network_info):
	    LOG.audit(str(network_info))

	if(image_meta.get('properties', {}).get('os_command_line')):
	    cmd = "bash -c 'java -jar " + full_name + " " + image_meta['properties'].get('os_command_line') + " > /dev/console'"
	else:
	    #cmd = ["java", "-jar", str(full_name)]
	    cmd = "bash -c 'java -jar " + full_name + " > /dev/console'"
	
	self._retrieve_and_inject_jar_in_container(context, container_id, image_meta['id'], self._encode_utf8(CONF.docker.docker_jar_path), full_name)
	
        LOG.audit("Command parameters: " + str(cmd))
        result = self.docker.execute(container_id, cmd, detach=True, stdout=False, stderr=False)
    ...
    def _execute_command(self, container, cmd, detach=False, stdout=True, stderr=True, stream=False, tty=False):
	...
	LOG.audit(data)
        ...
        # create the command
        url = self.docker._url('/containers/{0}/exec'.format(container))
        res = self.docker._post_json(url, data=data)
        self.docker._raise_for_status(res)
        ...
        res = self.docker._post_json(self.docker._url('/exec/{0}/start'.format(cmd_id)), data=data)
        self.docker._raise_for_status(res)
        if stream:
            return self.docker._multiplexed_socket_stream_helper(res)
        elif six.PY3:
            return bytes().join(
                [x for x in self.docker._multiplexed_buffer_helper(res)]
            )
        else:
            return str().join(
                [x for x in self.docker._multiplexed_buffer_helper(res)]
